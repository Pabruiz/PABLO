...
function setup_environment {
  ...
  URL="http://cau.urjc.es"
  SUPPORT="tu soporte local eduroam"
if [ ! -z "$DISPLAY" ] ; then
...
function confirm_exit {
  if [ ! -z $KDIALOG ] ; then
     if $KDIALOG --yesno "¿Deseas abandonar?"  ; then
     exit 1
     fi
  fi
  if [ ! -z $ZENITY ] ; then
     if $ZENITY --question --text="¿Deseas abandonar?" 2>/dev/null ; then
        exit 1
     fi
  fi
}
...
function user_cred {
  ...
  if ! USER_NAME=`prompt_nonempty_string 1 "introduce tu identificador de usuario"` ; then
    exit 1
  fi

  while [ "$PASSWORD" != "$PASSWORD1" ]
  do
    if ! PASSWORD=`prompt_nonempty_string 0 "introduce tu contraseña"` ; then
      exit 1
    fi
    if ! PASSWORD1=`prompt_nonempty_string 0 "repite tu contraseña"` ; then
      exit 1
    fi
...
setup_environment
show_info "Este instalador ha sido preparado para ${ORGANISATION}\n\nMás información y comentarios:\n\nEMAIL: ${SUPPORT}\nWWW: ${URL}\n\nInstalador creado con software del proyecto GEANT."
if ! ask "El instalador sólo funcionará correctamente si eres miembro de ${bf}Universidad Rey Juan Carlos.${n}" "Continuar" 1 ; then exit; fi
if [ -d $HOME/.cat_installer ] ; then
   if ! ask "El directorio $HOME/.cat_installer ya existe. Algunos de los ficheros en el mismo puede que sea sobrescritos." "Continuar" 1 ; then exit; fi
else
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:cibeles.urjc.es','DNS:deimos.urjc.es','DNS:europa.urjc.es','DNS:ganimedes.urjc.es','DNS:orion.urjc.es','DNS:radius.urjc.es','DNS:sirio.urjc.es','DNS:titan.urjc.es'})
        server_name = 'urjc.es'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
        ...
        s_8021x = dbus.Dictionary({
            ...
            'anonymous-identity': 'anonymous@urjc.es',
        })
...
network={
  ...
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
  anonymous_identity="anonymous@urjc.es"
}
EOFW
...
#prompt user for credentials
  ...
  if run_python_script ; then
   show_info "Instalación correcta"
else
   show_info "Falló la configuración de Network Manager, generando un fichero wpa_supplicant.conf"
   if ! ask "Network Manager configuration failed, but we may generate a wpa_supplicant configuration file if you wish. Be warned that your connection password will be saved in this file as clear text." "Escribir el archivo" 1 ; then exit ; fi

if [ -f $HOME/.cat_installer/cat_installer.conf ] ; then
  if ! ask "El fichero $HOME/.cat_installer/cat_installer.conf existe. Será sobrescrito." "Continuar" 1 ; then confirm_exit; fi
  rm $HOME/.cat_installer/cat_installer.conf
  fi
   create_wpa_conf
   show_info "Salida guardada en $HOME/.cat_installer/cat_installer.conf"
fi
